...
export class Memory {
    ...
    normalizeSession(session) {
        ...
        return {
            ...
            tokenUsage: {
                ...
                ...session.tokenUsage,
                byAgent: session.tokenUsage?.byAgent ?? {},
            },
        };
    }
    getBudgetLevel(phase) {
    ...
    buildBudgetAdvice(status) {
        if (status === 'healthy')
            return 'Keep the context narrow. Load only the files needed for the next step.';
        if (status === 'warning')
            return 'Context is getting expensive. Prefer a snapshot, smaller tasks, or a fresh handoff.';
    ...
    resolveEstimateFiles(phase, taskHint) {
        const hint = (taskHint ?? '').toLowerCase();
        const files = new Set([
            '.citadel/skills/rules_essential.md',
            `${PROJECT_HUB_DIR}/${HUB_FILES.status}`,
        ]);
        if (hint || phase !== 'inception')
            files.add(`${PROJECT_HUB_DIR}/${HUB_FILES.context}`);
        if (hint)
            files.add(`${PROJECT_HUB_DIR}/${HUB_FILES.handoff}`);
        if (phase === 'inception' || phase === 'specification') {
    ...
    buildEstimateAdvice(status) {
        if (status === 'healthy')
            return 'Estimated context is under control. You can proceed, but still load only what is needed.';
        if (status === 'warning')
            return 'Estimated context is getting heavy. Split the task or skip non-essential files before asking the model to act.';
        return 'Estimated context is too heavy for a clean next turn. Snapshot, narrow the task, or start a fresh focused session.';
    }
    summarizeTokenUsage(session) {
    ...
    estimateContextPressure(taskHint) {
    ...
    syncStatusFile(session, estimate = this.estimateContextPressure()) {
    ...
    syncTokenFile(session, estimate = this.estimateContextPressure()) {
    ...
    // ── Session ──
    getSession() { return this.normalizeSession(this.read('session.json', null)); }
    getTokenSummary() {
        const session = this.getSession();
        return session ? this.summarizeTokenUsage(session) : null;
    }
    getContextEstimate(taskHint) {
        return this.estimateContextPressure(taskHint);
    }
    refreshProjectHub(taskHint) {
        const session = this.getSession();
        if (!session)
            return;
    ...
    recordTokenUsage(agentId, tokensUsed, model) {
