...
export class GateSystem {
  ...
  initGate(id: GateId): GateState { const d = G[id]; if (!d) throw new Error(`Unknown gate: ${id}`); const g: GateState = { id, name:d.name, status:'pending', blocker:d.blocker, signoffs:{}, checks: d.checks.map(c => ({...c, status:'pending' as const, timestamp:null, notes:''})) }; this.sv(g); return g; }
  initAllGates(): void { Object.keys(G).forEach(id => this.initGate(id as GateId)); }
  getGate(id: GateId): GateState { return this.ld(id); }
  passCheck(gid: GateId, cid: string, notes=''): GateState { const g = this.ld(gid); const c = g.checks.find(x=>x.id===cid); if (c) { c.status='passed'; c.timestamp=new Date().toISOString(); c.notes=notes; } g.status = g.checks.every(x=>x.status==='passed') ? 'passed' : 'in-progress'; this.sv(g); return g; }
  failCheck(gid: GateId, cid: string, notes: string): GateState { const g = this.ld(gid); const c = g.checks.find(x=>x.id===cid); if (c) { c.status='failed'; c.timestamp=new Date().toISOString(); c.notes=notes; } g.status='failed'; this.sv(g); return g; }
  vetoGate(gid: GateId): GateState { const g = this.ld(gid); g.status='blocked'; g.signoffs['ciso']=false; this.sv(g); return g; }
  canProceed(gid: GateId): { allowed: boolean; blockers: string[] } { const g = this.ld(gid); const b: string[] = []; if (g.status==='blocked') return {allowed:false,blockers:['BLOCKED by CISO']}; g.checks.filter(c=>c.status!=='passed').forEach(c=>b.push(`[${c.status}] ${c.description}`)); if (gid==='gate-3'&&!g.signoffs['ciso']) b.push('CISO not signed off'); return {allowed:b.length===0,blockers:b}; }
  getProgressSummary(gid: GateId): string { const g = this.ld(gid); const p = g.checks.filter(c=>c.status==='passed').length; let s = `\n🚧 ${g.name} — ${p}/${g.checks.length} (${g.status})\n`; g.checks.forEach(c => { const i = c.status==='passed'?'✅':c.status==='failed'?'❌':'⬜'; s += `  ${i} ${c.description}\n`; }); if (g.status!=='passed') s += `  ⛔ ${g.blocker}\n`; return s; }
}
