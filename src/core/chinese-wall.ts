...
export class ChineseWall {
   ...
   */
  static canReview(makerId: string, checkerId: string): { allowed: boolean; violation?: WallViolation } {
  ...
  static getValidCheckers(makerId: string): string[] {
    const maker = getAgent(makerId);
    if (!maker || maker.level !== 'maker') return [];
    const checkers = getCheckersByTeam(maker.team as TeamId);
   ...
   */
  static validateReviewChain(assignments: Array<{ maker: string; checker: string }>): WallViolation[] {
